...
const OurStory = () => {
	return (
		<section className='pt-24 max-lsm:pt-6 max-md:pt-16 max-xlsm:pt-8'>
			<Container className='grid grid-cols-2 gap-5 max-md:grid-cols-1'>
				<div>
					<div className='h-7 w-7 bg-main mb-6'></div>
					<h4 className='font-medium [letter-spacing:3px] uppercase text-gray-500 mb-4'>
						Our Story 👇{' '}
					</h4>
					<h2 className='font-semibold text-[48px] leading-[1.2] text-darkBlue max-md:text-[36px] max-lsm:text-[28px] max-[425px]:text-[32px] max-[320px]:text-[27px] mb-6 max-w-[549px] max-lsm:mb-4'>
				...
				<div className='bg-siteOrange/60 flex items-center justify-center p-5'>
					<div className='grid grid-cols-2 gap-[62px] max-lsm:grid-cols-1'>
						{stats.map(stat => (
							<StatCard
								key={stat.value}
								value={stat.value}
