...
const ServiceHeader = () => {
	return (
		<header className='service-header py-24 max-[425px]:py-[30px] bg-siteOrange'>
			<div className='max-w-[1320px] w-full px-5 mx-auto grid md:grid-cols-1 md:gap-6' style={{ gridTemplateColumns: '1.2fr 1fr' }}>
				...
				<div className='service-header-right flex items-center justify-center max-[475px]:justify-start'>
					<ul className='service-header-links flex flex-col gap-4'>
						{serviceHeaderLinks.map(link => (
							<li key={link}>
								<Link className='service-header-link font-semibold text-siteDark flex items-center gap-3 text-[24px] leading-[1.5] hover:[&>.icon]:inline-block max-[475px]:items-start max-[475px]:text-[18px] max-[475px]:leading-[1.2]' to=''>
									{link}
